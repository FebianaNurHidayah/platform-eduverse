        ...
        function normalizeEmailOrPhone(input) {
        ...
        async function handleLogin(event) {
            ...
            try {
                ...
                const normalizedInput = normalizeEmailOrPhone(emailOrPhone);
                let user = allUsers.find(u => u.email_or_phone === normalizedInput && u.password === password);
                
                if (!user) {
                    showNotification('Email/nomor telepon atau password salah!', 'error');
                    return;
                }
                ...
                if (lastLoginDate !== today) {
                    const challenges = JSON.parse(user.daily_challenges || '[false,false,false,false,false]');
                    
                    for (let i = 0; i < challenges.length; i++) {
                        if (!challenges[i]) {
                            challenges[i] = true;
                            user.total_stars = (user.total_stars || 0) + 50;
                            showNotification('🎁 Daily Challenge bonus: +50 Stars!', 'success');
                            break;
                        }
                    }
                    
                    user.daily_challenges = JSON.stringify(challenges);
                    user.last_login_date = today;
                }
                
                user.last_active = new Date().toISOString();
                const updateResult = await window.dataSdk.update(user);
                ...
                closeModal();
                showNotification(`👋 Selamat datang kembali, ${user.full_name}!`, 'success');
        ...
        async function handleSignup(event) {
            ...
            try {
                ...
                const normalizedInput = normalizeEmailOrPhone(emailOrPhone);
                let existingUser = allUsers.find(u => u.email_or_phone === normalizedInput);
                ...
                const newUser = {
                    full_name: fullName,
                    email_or_phone: normalizedInput,
                    password: password,
                    total_stars: 50,
                    daily_challenges: JSON.stringify([true, false, false, false, false]),
                ...
                if (!createResult.isOk) {
                    showNotification('Gagal membuat akun baru. Silakan coba lagi.', 'error');
                    return;
                }
        ...
        function checkUserClass() {
            ...
            const user = allUsers.find(u => u.full_name === currentUser);
            if (user && user.selected_class) {
        ...
        async function selectClass(classLevel) {
            ...
            const user = allUsers.find(u => u.full_name === currentUser);
            if (user) {
                user.selected_class = classLevel;
                const updateResult = await window.dataSdk.update(user);
                if (updateResult.isOk) {
                    showSelectedClassInfo();
                    showCategoriesSection();
                    ...
                    showNotification('❌ Gagal memilih kelas. Silakan coba lagi.', 'error');
                }
            } else {
        ...
        function updateUserInterface() {
            ...
            const user = allUsers.find(u => u.full_name === currentUser);
            if (user) {
                document.getElementById('userStars').textContent = user.total_stars || 0;
                updateDailyChallengesDisplay();
        ...
        async function finishQuiz() {
            ...
            const user = allUsers.find(u => u.full_name === currentUser);
            if (user) {
                user.total_stars = (user.total_stars || 0) + starsEarned;
                
                const quizHistory = JSON.parse(user.quiz_history || '[]');
                quizHistory.push({
                ...
                user.quiz_history = JSON.stringify(quizHistory);
